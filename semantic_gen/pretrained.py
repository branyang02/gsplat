...
import viser
import nerfview
from datasets.colmap import Parser, DynamicDataset
...
class Config:
    ...
    eval_steps: List[int] = field(
        default_factory=lambda: [1_000, 3_000, 7_000, 14_000, 30_000]
    )
...
class Runner:
    ...
    def __init__(self, cfg: Config, ckpt) -> None:
        set_random_seed(42)
        ...
        self.sam_module, cfg.sh_degree = self._get_sam_module(ckpt["sam_module"])
    ...
    def train(self):
        ...
        # Before training, evaluate the pretrained model.
        self.eval(step=0)
...
def main(cfg: Config, ckpt: Dict):
    runner = Runner(cfg, ckpt)
    runner.train()
